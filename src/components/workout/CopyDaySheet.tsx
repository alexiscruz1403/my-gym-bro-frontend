...
import { copyPlanDay } from '@/services/workout-plans.service';
...
export function CopyDaySheet({ planId, dayOfWeek, open, onOpenChange }: CopyDaySheetProps) {
  ...
  async function handleCopy() {
    ...
    try {
      if (mode === 'existing' && selectedPlanId) {
        const targetPlan = plans.find((p) => p.id === selectedPlanId);
        await copyPlanDay(planId, dayOfWeek, { targetPlanId: selectedPlanId });
        ...
        await copyPlanDay(planId, dayOfWeek, { newPlanName: trimmedName });
    ...
    <Sheet open={open} onOpenChange={handleClose}>
      <SheetContent side="bottom" className="flex flex-col max-h-[92dvh] rounded-t-[20px]">
        ...
        <SheetHeader className="shrink-0 px-5 pb-1 pt-3">
          <SheetTitle>{t('plans.copyDaySheet.title')}</SheetTitle>
          <SheetDescription className="sr-only">{t('plans.copyDaySheet.title')}</SheetDescription>
        </SheetHeader>
          ...
          <div className="grid grid-cols-2 gap-2">
            {(['existing', 'new'] as const).map((m) => (
              <button
                key={m}
                type="button"
                onClick={() => { setMode(m); setSelectedPlanId(null); }}
                className={cn(
              ...
              >
                {m === 'existing'
                  ? t('plans.copyShared.existingPlan')
                  : t('plans.copyShared.newPlan')}
              </button>
            ))}
          </div>
